package main.states;

import java.util.List;

import javax.swing.JButton;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JTextField;

import main.DatabaseManager;
import main.models.User;

public class UserListState extends ListState<User> {
    private String search = "";

    @Override
    protected String getTitle() {
        return "Users";
    }

    @Override
    protected List<User> fetchItems() {
        return DatabaseManager.getInstance().getUsers(search, user);
    }

    @Override
    protected String[] getColumns() {
        return new String[]{"Name", "Type", "Status"};
    }

    @Override
    protected Object[] getRowValues(User u) {
        return new Object[]{u.getName(), u.getType(), u.getStatus()};
    }

    @Override
    protected void onSelect(User u) {
        next(new UserDetailState(u));
    }

    @Override
    protected void renderExtras(JPanel extrasPanel) {
        JTextField txtSearch = new JTextField(search, 18);
        JButton btnSearch = new JButton("Search");

        Runnable applySearch = () -> {
            search = txtSearch.getText().trim();
            refresh();
        };

        txtSearch.addActionListener(e -> applySearch.run());
        btnSearch.addActionListener(e -> applySearch.run());

        extrasPanel.add(new JLabel("Search:"));
        extrasPanel.add(txtSearch);
        extrasPanel.add(btnSearch);
    }
}